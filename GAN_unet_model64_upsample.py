...
class GANUnetModel64_upsample():
    ...
    def __init__(self):
        ...
        self.output_shape_d_v2 = (4, 4, 4) + (self.channels,)
    ...
    def build_generator(self):
        ...
        # downsampling
        down1 = conv3d(input_tensor=combined_imgs, n_filters=self.gf, padding='valid', name='down1_1', batch_normalization=False)  # 62x62x62
        ...
        up = Cropping3D(4)(UpSampling3D(size=3)(up1)) # 64x64x64

        phi = Conv3D(filters=3, kernel_size=(1, 1, 1), strides=1, name='phi', padding='same', activation='tanh')(up)
    ...
    def build_discriminator_v2(self):
        ...
        def d_layer(layer_input, filters, f_size=3, bn=True, name=''): #use kernal 3
            """Discriminator layer"""
        ...
        img_A =  Input(shape=self.input_shape_d, name='input_img_A')             # 64x64x64 warped_img or reference
        img_T = Input(shape=self.input_shape_g, name='input_img_T')             # 64x64x64 template

        #img_T_cropped = Cropping3D(cropping=20)(img_T)  # 24x24x24

        # Concatenate image and conditioning image by channels to produce input
        combined_imgs = Concatenate(axis=-1)([img_A, img_T])
        ...
        validity = Conv3D(1, kernel_size=3, strides=1, padding='same', activation='sigmoid', name='disc_sig')(d4) # 4x4x4
    ...
    def build_transformation(self):
        ...
        phi = Input(shape=self.output_shape_g, name='input_phi_transform')     # 64x64x64
    ...
    def train(self, epochs, batch_size=1, sample_interval=50):
        ...
        for epoch in range(epochs):
            for batch_i, (batch_img, batch_img_template) in enumerate(self.data_loader.load_batch()):
                ...
                phi = self.generator.predict([batch_img, batch_img_template]) #64x64x64
                assert not np.any(np.isnan(phi))

                # deformable transformation
                transform = self.transformation.predict([batch_img, phi])     #64x64x64
                ...
                if noisy_prob < 0.85: # occasionally flip labels to introduce noisy labels
                    d_loss_real = self.discriminator.train_on_batch([batch_ref, batch_img_template], validhard)
                    d_loss_fake = self.discriminator.train_on_batch([transform, batch_img_template], fakehard)
                else:
                    d_loss_real = self.discriminator.train_on_batch([batch_ref, batch_img_template], fakehard)
