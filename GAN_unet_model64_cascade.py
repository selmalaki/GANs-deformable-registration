...
class GANUnetModel64Cascade():
    ...
    def __init__(self):
       ...
       # Build the deformable transformation layer
        ...
        # Build the deformable transformation layer
        self.transformation_seg = self.build_transformation_seg()
        self.transformation_seg.summary()
        ...
        if self.DEBUG:
            log_path = '/nrs/scicompsoft/elmalakis/GAN_Registration_Data/flydata/forSalma/lo_res/logs_ganunet_cascade/'
        ...
        self.segmentation_model = load_model('/nrs/scicompsoft/elmalakis/GAN_Registration_Data/flydata/forSalma/lo_res/generated_unet_nogap/gan_network100.gen.h5')
    ...
    def build_generator(self):
        ...
        img_S = Input(shape=self.input_shape_g, name='input_img_S')                                       # 64x64x64
        img_T = Input(shape=self.input_shape_g, name='input_img_T')                                       # 64x64x64
    ...
    """
    Deformable Transformation Layer for segmentation  
    """
    def build_transformation_seg(self):
        ...
        warped_S = Cropping3D(4)(UpSampling3D(size=3)(warped_S))  # 64x64x64

        return Model([img_S, phi], warped_S,  name='transformation_layer_seg')
    ...
    def train(self, epochs, batch_size=1, sample_interval=50):
        ...
        for epoch in range(epochs):
            for batch_i, (batch_img, batch_img_template, batch_img_golden) in enumerate(self.data_loader.load_batch()):
                ...
                phiseg = self.segmentation_model.predict([batch_img, batch_img_template])
                batch_img_seg = self.transformation_seg.predict([batch_img, phiseg]) #64x64x64

                # casecade the deformation network
                phi = self.generator.predict([batch_img_seg, batch_img_template]) #24x24x24
                #assert not np.any(np.isnan(phi))

                # deformable transformation
                transform = self.transformation.predict([batch_img_seg, phi])     #24x24x24
                ...
                batch_ref = perturbation_factor_alpha * batch_img_seg + (1- perturbation_factor_alpha) * batch_img_template #64x64x64

                batch_img_sub = np.zeros((self.batch_sz, output_sz, output_sz, output_sz, self.channels), dtype=batch_img_seg.dtype)
                ...
                batch_img_sub[:, :, :, :, :] = batch_img_seg[:, 0 + gap:0 + gap + output_sz,
                ...
                g_loss = self.combined.train_on_batch([batch_img_seg, batch_img_template], validhard)
    ...
    def sample_images(self, epoch, batch_i):
        ...
        os.makedirs(path+'generated_cascade//' , exist_ok=True)
        ...
        nrrd.write(path+"generated_cascade//%d_%d_%d" % (epoch, batch_i, idx), predict_img)
        ...
        gan.combined.save(path+ 'generated_cascade//'+ file_name + '.whole.h5', overwrite=True)
        print('Save the whole network to disk as a .whole.h5 file')
        model_jason = gan.combined.to_json()
        with open(path+ 'generated_cascade//'+file_name + '_arch.json', 'w') as json_file:
            json_file.write(model_jason)
        gan.combined.save_weights(path+ 'generated_cascade//'+file_name + '_weights.h5', overwrite=True)
        print('Save the network architecture in .json file and weights in .h5 file')

        # save the generator network
        gan.generator.save(path+ 'generated_cascade//'+file_name + '.gen.h5', overwrite=True)
        print('Save the generator network to disk as a .whole.h5 file')
        model_jason = gan.combined.to_json()
        with open(path+ 'generated_cascade//'+file_name + '_gen_arch.json', 'w') as json_file:
            json_file.write(model_jason)
        gan.combined.save_weights(path+ 'generated_cascade//'+file_name + '_gen_weights.h5', overwrite=True)
