...
class GANUnetModel64():
    ...
    def __init__(self):
       ...
       # Build the deformable transformation layer
        ...
        if self.DEBUG:
            log_path = '/nrs/scicompsoft/elmalakis/GAN_Registration_Data/flydata/forSalma/lo_res/logs_ganunet_withref/'
    ...
    def sample_images(self, epoch, batch_i):
        ...
        os.makedirs(path+'generated_ganunet_withref/' , exist_ok=True)
        ...
        gan.combined.save(path+ 'generated_ganunet_withref/'+ file_name + '.whole.h5', overwrite=True)
        print('Save the whole network to disk as a .whole.h5 file')
        model_jason = gan.combined.to_json()
        with open(path+ 'generated_ganunet_withref/'+file_name + '_arch.json', 'w') as json_file:
        ...
        with open(path+ 'generated_ganunet_withref/'+file_name + '_gen_arch.json', 'w') as json_file:
            json_file.write(model_jason)
        gan.combined.save_weights(path+ 'generated_ganunet_withref/'+file_name + '_gen_weights.h5', overwrite=True)
