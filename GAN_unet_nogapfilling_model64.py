...
class GANUnetNoGapFillingModel64():
    ...
    def __init__(self):
       ...
       # Build the deformable transformation layer
        ...
        self.combined.compile(loss = [partial_gp_loss, 'mae'] , optimizer=optimizerG)
    ...
    def build_generator(self):
        ...
        up2 = conv3d(input_tensor=up2, n_filters=2 * self.gf, padding='valid', name='up2_1')  # 16x16x16
        up2 = conv3d(input_tensor=up2, n_filters=2 *self.gf, padding='valid', name='up2_2')  # 14x14x14

        up1 = concatenate([Cropping3D(((16, 16), (16, 16), (16, 16)))(down1), UpSampling3D(size=(2, 2, 2))(up2)])  # 28x28x28
        up1 = conv3d(input_tensor=up1, n_filters=self.gf, padding='valid', name='up1_1')  # 26x26x26
        up1 = conv3d(input_tensor=up1, n_filters=self.gf, padding='valid', name='up1_2')  # 24x24x24
    ...
    def build_discriminator(self):
        ...
        validity = Conv3D(1, kernel_size=1, strides=1, activation='sigmoid', name='validity')(d5) #6x6x6
    ...
    def train(self, epochs, batch_size=1, sample_interval=50):
        ...
        for epoch in range(epochs):
            for batch_i, (batch_img, batch_img_template, batch_img_golden) in enumerate(self.data_loader.load_batch()):
                ...
                #  Train Generator
                # ---------------------
                g_loss = self.combined.train_on_batch([batch_img, batch_img_template], [validhard, batch_golden_sub])
    ...
    def sample_images(self, epoch, batch_i):
        ...
        gan.combined.save(path+ 'generated_unet_nogap/'+ file_name + '.whole.h5', overwrite=True)
        ...
        gan.combined.save_weights(path+ 'generated_unet_nogap/'+file_name + '_weights.h5', overwrite=True)
        print('Save the network architecture in .json file and weights in .h5 file')

        # save the generator network
        gan.generator.save(path+ 'generated_unet_nogap/'+file_name + '.gen.h5', overwrite=True)
        ...
        with open(path+ 'generated_unet_nogap/'+file_name + '_gen_arch.json', 'w') as json_file:
            json_file.write(model_jason)
        gan.combined.save_weights(path+ 'generated_unet_nogap/'+file_name + '_gen_weights.h5', overwrite=True)
...
if __name__ == '__main__':
    ...
    gan = GANUnetNoGapFillingModel64()
